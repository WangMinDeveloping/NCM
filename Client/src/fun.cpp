...
void regropFilePackage(unsigned char *pFileBytes, int iFileBytes) // 组文件包
{
	...
	pSocketSeq->uMaxRecvAck = pSocketSeq->uNextSendSeq; //这个可以说是包序号,每组一次包就加1，最后为文件总报数
	...
	if (pSocketSeq->uSendPackageCount != 0) // 可发送再发送，uSendPackageCount标识一次性可以发送的数据量
	{
		if (pSocketSeq->uSendPackageCount == pSocketSeq->uMaxSendPackageCount)
			(pSIpData->TcpHead).wAckFirstPack = 1;   
...
//sack 代表是区间确认，收到该包约等于存在丢包
unsigned int sackHandle(struct sIpData *pIPData)
{
	int groups = (pIPData->UserHead.wParametersLength - 24) / 8;//这里是bug？ 为什么要减24.
	...
	unsigned int uMinACK = pSocketSeq->uNextRecvAck;//目前期望的收到的ack值

	pSocketSeq->uMaxSendPackageCount--; // 收到ACK减少最大发送包数量
	while (groups != 0)
	{
		unsigned int uMaxACK = pReceiveACK->uMinAckNumber;
		for (unsigned int i = uMinACK; i < uMaxACK; i++) //uMinACK 和 uMaxACK之间的序号是丢失的包
		{
...
unsigned int ackHandle(struct sIpData *pIPData)
{
	...
	if (pIPData->TcpHead.uAckNumber >= pSocketSeq->uNextRecvAck)
	{
		...
		unsigned int uSendMaxSeq = pIPData->TcpHead.uAckNumber + pSocketSeq->uSendPackageCount;
		//根据滑动窗口发送文件包
		for (unsigned int i = pIPData->TcpHead.uAckNumber + 1; i < uSendMaxSeq; i++)
		{
			pstResendPackage = pFilePackage->find(i);
			if (pstResendPackage != NULL)
			{
